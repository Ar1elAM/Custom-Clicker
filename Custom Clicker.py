...
set_XY_key = KeyCode(char=input(
    "Please choose a key to set custom mouse positions:"))
start_stop_key = KeyCode(char=input(
    "Please choose a key to start/stop mouse movement:"))
...
def key_press(key):
    ...
    if key == set_XY_key:
        if (custom == True):
            current_mouse_pos = mouse.position
            mouse_pos.append(current_mouse_pos)  # type: ignore
            mouse_time.append(float(input(
                "\nPlease choose a value(in seconds) for the time you wish to wait between actions:")))  # type: ignore
        else:
            custom = True
            print("\nYou chose custom mode, this mode is useful for precise clicks positioning.\nJust hover your mouse where you want it to click, and press the ", set_XY_key,
                  " key, and type the amount of seconds you want it to wait.\nWhen ready just press the button you chose as " '"start button": ', start_stop_key, " key.")
        ...
        if auto_thread.running:
            ...
            print("\nAt any moment you can press the ", quit_key, " key to close the script, or the ", start_stop_key,
